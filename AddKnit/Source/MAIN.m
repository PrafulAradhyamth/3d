...
% ask Uwe for details :)
W = [0.96, 0.88, 0.84, 0.76, 10/6, 50/6, 1, 10];
H = [0.42, 0.3, 0.85, 0.44, 10/13, 50/13, 0.5, 5];
...
% 5 Knoten = 1, Kanten = 2
% 4 Index of node or edge (depends on column 5)
% 5 nodes = 1, edges = 2
...
if usrseam
    % if seam is present, seam and centerline are assigned inslines
    inslines = [lns{3};midpts];
    % Length (number of points) of the seam
    seamlen = size(lns{3},1);
else
    % without seam, centerline is assigned inslines
    inslines = midpts;    
    seamlen = 0;
end
%% if seam or centerline is present (if inslines is not empty)
if ~isempty(inslines)
    %% - % Seam is inserted
...
%% plot time field
% It is formed in the area of embroidery time over the whole body, 
% in equal sections with 0.05
...
if ~isempty(seam)
    % If there is a seam, the points are doubled and the mesh on of the seam location cut through
