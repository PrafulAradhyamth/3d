...
#define __OPC_TREEBUILDERS_H__
	...
	class OPCODE_API AABBTreeOfVerticesBuilder : public AABBTreeBuilder
	{
		...
		overrides(AABBTreeBuilder)	bool			ComputeGlobalBox(const udword* primitives, udword nb_prims, IceMaths::AABB& global_box)	const;
		overrides(AABBTreeBuilder)	float			GetSplittingValue(udword index, udword axis)									const;
		overrides(AABBTreeBuilder)	float			GetSplittingValue(const udword* primitives, udword nb_prims, const IceMaths::AABB& global_box, udword axis)	const;
