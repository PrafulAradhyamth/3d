function [VV,FF,I,J] = remove_degenerate_faces(V,F,varargin)
  ...
  [V1,FF,~,J] = faces_first(V,F,boundary_faces(F));
  EJ = (1:size(V1,1))';
  ...
  while true
    [IA,~,l,A] = is_acute(V1,FF);
    D = A/2 < epsilon;
    % Find non-delaunay, longest edges of small, non-acute triangles
    L = ~is_delaunay(V1,FF) & bsxfun(@eq,l,max(l,[],2)) & repmat(D&~IA,1,3);
  ...
  [VV,UJ,I] = remove_unreferenced(V1,FF);
  FF = UJ(FF);
  J = UJ(EJ(J));
